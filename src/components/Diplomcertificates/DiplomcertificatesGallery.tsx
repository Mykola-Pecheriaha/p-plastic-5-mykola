...
export function DiplomcertificatesGallery({ images }: DiplomcertificatesGalleryProps) {
  ...
  // Покращений обробник коліщатка миші для зуму
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      if (fullscreen && fullscreenRef.current?.contains(e.target as Node)) {
        ...
        const delta = e.deltaY > 0 ? -0.3 : 0.3 // Збільшую крок зуму
        setZoomLevel(prev => {
          const newZoom = Math.max(0.5, Math.min(5, prev + delta))
          console.log("Zoom level:", newZoom) // Для дебагу
          return newZoom
        })
      }
    }
  ...
  const nextImage = () => {
    setCurrentIndex(prevIndex => (prevIndex + 1) % images.length)
    setZoomLevel(1) // Скидаємо зум при зміні зображення
  }
  ...
  // Покращені функції зуму
  const zoomIn = () => {
    setZoomLevel(prev => {
      const newZoom = Math.min(prev + 0.5, 5)
      console.log("Zoom in:", newZoom) // Для дебагу
      return newZoom
    })
  }
  ...
  const zoomOut = () => {
    setZoomLevel(prev => {
      ...
      console.log("Zoom out:", newZoom) // Для дебагу
      return newZoom
    })
  }
    ...
    <div className={styles.galleryContainer}>
      <div className={styles.gallery}>
        ...
        >
          ...
          <Image
            src={currentImage.src || "/placeholder.svg?height=800&width=600&query=diploma certificate"}
        ...
        <div className={styles.bottomPanel}>
          <div className={styles.thumbnailsRow}>
            {images.map((image, index) => (
              ...
              >
                <Image
                  src={image.src || "/placeholder.svg?height=80&width=60&query=diploma certificate thumbnail"}
                  alt={image.alt}
                  width={60}
                  height={80}
            ...
            {showThumbnails && (
              <div className={styles.thumbnailsContainer}>
                {images.map((image, index) => (
                  ...
                  >
                    <Image
                      src={image.src || "/placeholder.svg?height=75&width=100&query=diploma certificate thumbnail"}
                      alt={image.alt}
                      width={100}
                      height={75}
