...
import styles from "./GernioFAQGallery.module.css"
...
interface GernioFAQGalleryProps {
  images?: GalleryImage[]
}

export function GernioFAQGallery({ images = [] }: GernioFAQGalleryProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>([])

  // Безпечна перевірка масиву зображень з useMemo
    ...
    <div className={styles.galleryContainer}>
      <div className={styles.gallery}>
        ...
        <div className={styles.imageContainer}>
          ...
          <Image
            src={currentImage.src || "/placeholder.svg?height=600&width=800&query=gernio faq"}
            ...
            {showThumbnails && (
              <div className={styles.thumbnailsContainer}>
                {safeImages.map((image, index) => (
                  ...
                  >
                    <Image
                      src={image.src || "/placeholder.svg?height=75&width=100&query=gernio faq thumbnail"}
                      alt={image.alt}
                      width={100}
                      height={75}
...
export default GernioFAQGallery
